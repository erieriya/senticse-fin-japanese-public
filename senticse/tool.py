...
class SimCSE(object):
    ...
    def __init__(self, model_name_or_path: str, 
        ...
        self._senticse_model = hasattr(config, "senticse_args") or any(
            name in {"BertForCL", "RobertaForCL", "ElectraModelForCL"}
            for name in (config.architectures or [])
        )
        if self._senticse_model:
            from .models import BertForCL, RobertaForCL, ElectraModelForCL
            model_class = {"bert": BertForCL, "roberta": RobertaForCL, "electra": ElectraModelForCL}[config.model_type]
    ...
    def encode(self, sentence: Union[str, List[str]], 
        ...
        with torch.no_grad():
            ...
            for batch_id in tqdm(range(total_batch)):
                ...
                kwargs = {"sent_emb": True} if self._senticse_model else {}
